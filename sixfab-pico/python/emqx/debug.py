...
class Debug:
    ...
    def print(self, *args):
        ...
        if self.debug_enabled:
            if self.debug_channel == DebugChannel.USBC:
                ...
                self.uart1.write("\n")
            # Log file anyway
            time_stamp = "{:04d}-{:02d}-{:02d}-{:02d}-{:02d}-{:02d}".format(*time.localtime()[0:6])
            self.add_log("/tmp/log.txt", str(time_stamp) + " ")
            for arg in args:
                self.add_log("/tmp/log.txt", str(arg))
                self.add_log("/tmp/log.txt", " ")
            self.add_log("/tmp/log.txt", "\n")
    ...
    def add_log(self, file_path, data):
